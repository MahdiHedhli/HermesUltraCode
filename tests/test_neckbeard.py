"""Acceptance criterion 8: the neckbeard ruleset is present and injected into the
...
class NeckbeardRulesetTest(unittest.TestCase):
    def test_ruleset_present(self):
        text = load_ruleset()
        self.assertIn("Neckbeard ruleset (vendored)", text)
    ...
    def test_inject_into_orchestrator_full(self):
        ...
        self.assertIn("Neckbeard ruleset (vendored)", out)
    ...
    def test_inject_into_orchestrator_lite(self):
        ...
        self.assertIn("Neckbeard (lite)", out)
        self.assertIn("BASE TASK", out)
    ...
    def test_worker_always_full_even_if_lite_requested(self):
        ...
        self.assertIn("Neckbeard ruleset (vendored)", out)  # full text, not lite

    def test_reviewer_injection_forbidden(self):
        with self.assertRaises(NeckbeardError):
    ...
    def test_no_marketplace_plugin_or_node_hooks_referenced(self):
        ...
        banned = ("marketplace plugin", "marketplace", "npx neckbeard", "neckbeard-plugin",
                  "lifecycle hook", "node lifecycle")
        offenders = []
        for dirpath, dirnames, filenames in os.walk(REPO_ROOT):
            dirnames[:] = [d for d in dirnames if d not in {".git", "node_modules", "__pycache__"}]
            for name in filenames:
                if name == "test_neckbeard.py":
    ...
    def test_no_package_json_depends_on_neckbeard_plugin(self):
            ...
            for name in filenames:
                if name in ("package.json", "package-lock.json"):
                    with open(os.path.join(dirpath, name), encoding="utf-8") as fh:
                        self.assertNotIn("neckbeard", fh.read().lower())

    def test_harvest_markers_finds_debt(self):
        items = harvest_markers(REPO_ROOT)
        # We deliberately mark shortcuts with `neckbeard:`; at least a few exist.
